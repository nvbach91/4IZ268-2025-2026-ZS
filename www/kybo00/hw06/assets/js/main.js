...
const caesarDecipher = (cipherText, usedKey) => {
    ...
    const deciphered = shiftString(cipherText, usedKey);
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const cipherOutputText = document.getElementById('cipher-output');
    
    const button = document.getElementById('decipher-button');

button.addEventListener("click", () => {
    const text = cipherTextInput.value;
    const key = parseInt(cipherKeyInput.value.trim())

    const result = caesarDecipher(text, key);
    cipherOutputText.textContent = result;
