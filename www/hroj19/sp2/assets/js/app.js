...
const App = {
    ...
    form.addEventListener("submit", async (e) => {
      ...
      try {
        ...
        if (searchData.searchMode === "ingredient" && searchData.alcoholFilter !== "all") {
          showResultsInfo("Type filter can't be applied to ingredient search results (API returns limited data).");
        } else {
          clearResultsInfo();
        }
  ...
  createResultItem(drink) {
    ...
    rowBtn.addEventListener("click", async (e) => {
      ...
      if (favBtn) {
        ...
        App.toggleFavourite(drink.idDrink);

        // re-render results so star icon updates
        App.renderResults(App.lastResults);
  ...
  isFavourite(idDrink) {
    return App.favourites.includes(String(idDrink));
  },

  // toggles favourite on/off and persists it to localStorage
  toggleFavourite(idDrink) {
    const id = String(idDrink);

    if (App.isFavourite(id)) {
      App.favourites = App.favourites.filter((x) => x !== id);
    } else {
      App.favourites.push(id);
    }
