...
document.querySelector('#decipherBtn').addEventListener('click', () => {
    const text = document.querySelector('#cipherText').value;
    const key = parseInt(document.querySelector('#key').value);
    const outputDiv = document.querySelector('#output');

    if (!text || isNaN(key)) {
        outputDiv.textContent = 'Zadej text a klíč!';
        return;
    }

    const deciphered = caesarDecipher(text, key);
    outputDiv.textContent = deciphered;
});