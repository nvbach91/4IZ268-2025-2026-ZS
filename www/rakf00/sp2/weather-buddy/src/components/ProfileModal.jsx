...
export default function ProfileModal({closeModal}) {
    ...
    return (<div id='modal-backdrop' onClick={closeModal}
                 className='cursor-pointer fixed inset-0 flex items-center justify-center bg-black/60 backdrop-blur-sm'>

        <div role='dialog' aria-modal='true' onClick={handleModalClick}
            ...
            <form onSubmit={handleSubmit(onSubmit)}>
                ...
                <Input min='0' name='age' placeholder='Age' label='Age' registration={register("age", {
                    required: "Field is required",
                    min: {
                        value: 1, message: 'Age must be positive number'
                    },
                    max: {value: 100, message: 'Max age is 1OO, if you are older, do not go outside'},
                    valueAsNumber: true
                })}
                />
                {errors.age && <p className='text-red-500 text-sm mt-[-10px] mb-2'>{errors.age.message}</p>}
                <Input name='height' min='1' placeholder='Height in cm'
                       label='Height' registration={register("height", {
                    required: "Field is required",
                    min: {
                        value: 1, message: 'Height be positive number'
                    },
                    max: {value: 230, message: 'Max height is 230cm, if you are taller, get a surgery'},
                    valueAsNumber: true
                })}/>
                {errors.height && <p className='text-red-500 text-sm mt-[-10px] mb-2'>{errors.height.message}</p>}
                <Input min='0' name='weight' placeholder='Weight in kg' label='Weight'
                       registration={register("weight", {
                           required: "Field is required", min: {
                               value: 1, message: 'Weight must be positive number'
                           }, max: {value: 200, message: 'Max weight is 200, are you a whale?'}, valueAsNumber: true
                       })}/>
                {errors.weight && <p className='text-red-500 text-sm mt-[-10px] mb-2'>{errors.weight.message}</p>}
                <p className='flex justify-between mb-1'><span className='font-bold'>Cold sensitivity</span>
                ...
                <div className='grid grid-cols-10 gap-2 relative'>
                    {[...Array(10)].map((_, i) => (<div key={i} className='relative'>
                            <Input
                                ...
                                type='radio'
                                name='sensitivity'
                                registration={register("sensitivity", {required: "How cold sensitive are you?", valueAsNumber:true})}
                                value={i + 1}
                ...
                {errors.sensitivity && <p className='text-red-500 text-sm  my-2'>{errors.sensitivity.message}</p>}
