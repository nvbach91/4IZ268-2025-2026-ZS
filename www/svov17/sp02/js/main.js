...
const App= {
    ...
    init:function() { //po nacteni stranky
        App.bindEvents(); 
...
// fce loadData
    ...
    loadQuestion:function() {
        ...
        $('#confirm-answer-btn').prop('disabled', true).text("Potvrdit odpověď").show(); //aby nebyla odeslana prazdna odpoved
    ...
    //fotka psa
        ...
        .done(function(response) {
            if (response.status=== 'success') {
                ...
                App.state.correctBreed=parts[parts.length - 2];
                
                const wrongBreeds= _.sampleSize(App.state.allBreeds, 3);
...
//fce submitAnswer
    submitAnswer:function() {
        ...
        if(isCorrect) {
            ...
            App.state.score++;
            $('#feedback-message').text("Správně!");
        } 
        else {
            selectedButton.addClass('wrong');
            $(`.option-btn[data-answer="${App.state.correctBreed}"]`).addClass('correct');
            $('#feedback-message').text(`Špatně! Správná odpověď je ${App.state.correctBreed}`);
        }
