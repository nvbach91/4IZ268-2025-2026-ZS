...
const shiftChar = (c, shift) => {
    ...
    const shiftedIndex = (alphabet.indexOf(c) + shift) % 26;
    const result = alphabet.charAt(shiftedIndex);
    return result;
};
...
const shiftString = (str, shift) => {
    let result = '';
    for (sign of str) {
        if (!alphabet.includes(sign)) {
            result += sign;
            continue;
        }
        const shiftedLetter = shiftChar(sign, shift);
...
const caesarDecipher = (cipherText, usedKey) => {
    ...
    return shiftString(upperText, 26 - usedKey);
...
cipherForm.addEventListener('submit', (event) => {
    ...
    const cipherText = formData.get('cipher');
    const usedKey = formData.get('key');
